...
- Building API Endpoints
  ...
  - Read through our API docs and build out endpoints for dogs and walks
...
This is a little picture of the react client for that will consuem the API that we'll be building over the next couple of lectures!
...
- Use separate Repos for frontend and backend 
- A monorepo is an option but it affects deployment
- There are pros and cons to each approach, but at this point it will be simpler to deploy your frontend to a service like netlify and your backend to heroku (should you choose to do so)
- Separate repos can make authentication a bit tricker to do
- In Phase 4, when we introduce authentication, we're going to deploy our rails application and our react application together from a single repo. So, you'll get some experience with both approaches.
...
Sinatra can render HTML, but we'll send back JSON instead so we can use React
